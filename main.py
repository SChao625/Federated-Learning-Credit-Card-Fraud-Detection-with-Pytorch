...
if __name__ == "__main__":
    ...
    accuracy_sum = accuracy_1994_1994 + accuracy_1995_1995 + accuracy_1996_1996 - 1.5
    weights = [
        abs((accuracy_1994_1994 - 0.5)) / accuracy_sum,
        abs((accuracy_1995_1995 - 0.5)) / accuracy_sum,
        abs((accuracy_1996_1996 - 0.5)) / accuracy_sum,
    ]
    # 放大最大权重
    amplification_factor = 2.0  # 放大因子，可以根据需要调整
    max_index = weights.index(max(weights))
    weights[max_index] *= amplification_factor
    ...
    # weights = [ 0.2,  0.2, 0.6]
    aggregated_params = aggregate_parameters(params_list, normalized_weights)

    server = Model()
    server.load_state_dict(aggregated_params)

    loss, accuracy = test(server, dataloader)
